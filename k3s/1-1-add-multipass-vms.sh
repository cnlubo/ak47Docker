...
# LastEditors: cnak47
# LastEditTime: 2022-09-23 10:47:31
...
nodeCount=1
read -r -p "How many worker nodes do you want to add?(default:$nodeCount) prompt with [ENTER]:" inputNode
nodeCount="${inputNode:-$nodeCount}"
cpuCount=2
read -r -p "How many cpus do you want per node?(default:$cpuCount) prompt with [ENTER]:" inputCpu
cpuCount="${inputCpu:-$cpuCount}"
memCount=4
read -r -p "How many gigabyte memory do you want per node?(default:$memCount) prompt with [ENTER]:" inputMem
memCount="${inputMem:-$memCount}"
diskCount=10
read -r -p "How many gigabyte diskspace do you want per node?(default:$diskCount) prompt with [ENTER]:" inputDisk
diskCount="${inputDisk:-$diskCount}"
read -r -p "Which Ubuntu version do you want to use? check multipass find (default:$OSversion) prompt with [ENTER]:" inputOSversion
OSversion="${inputOSversion:-$OSversion}"

num_Worker=$(multipass list | grep -c worker)
